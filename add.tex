...
It was first developed in the 1970s by the British National Research Development Corporation for Broadcasting purposes.\citationneeded Recently the format has found new popularity as higher order implementations in virtual reality applications and irregular mutlichannel setups.
...
This is a Placeholder for Jonas' math wisdom
...
The creation of a software library which can be implemented by plugin developers will enable widespread adoption of the file format. For software which doesn't include \textit{.add} functionality there are user facing tools for converting between file types. A list of compatible file types can be found in the \textit{.add} specification \autocite{spec}
